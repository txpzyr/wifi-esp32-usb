...
boolean FtpServer::processCommand()
{
    ...
    else if (!strcmp(command, "CWD"))
    {
        ...
        {
            if (parameters[0] == '/') sprintf(cwdName, parameters);
            else if(cwdName[strlen(cwdName)-1] == '/') sprintf(cwdName, (String(cwdName) + String(parameters)).c_str());
            else  sprintf(cwdName, (String(cwdName) + "/" + String(parameters)).c_str());
            client.println("250 Ok. Current directory is " + String(cwdName));
        }
    ...
    else if (!strcmp(command, "LIST"))
    {
        ...
        else
        {
            ...
            if (!root) {
                ...
                while (file) {
                    if (file.isDirectory()) {
                        //03-04-22  11:26PM       <DIR>          aa\r\n
                        data.println("03-04-22  11:26PM       <DIR>          " + String(file.name()));
                         //Serial.println("03-04-22  11:26PM       <DIR>          " + String(file.name()));
                        ...
                        String fs;
                        fs = String(file.size());
                        //12-20-21  12:53PM                 1345 tcpServer.py\r\n.nsp\r\n
                        data.println("12-20-21  12:53PM                 " + fs + " " + file.name());
                        //Serial.println(file.name());
                        nm++;
                    }
    ...
    else if (!strcmp(command, "MLSD"))
    {
        ...
        else
        {
            ...
            while (file) {
                if (file.isDirectory()) {
                    //Type = dir; Size = 0; Modify = 19700101000000; Perm = cdeflmp; UNIX.mode = 0777; atmosphere\r\n
                    data.println("Type=dir;Size=0;Modify=19700101000000;Perm=cdeflmp;UNIX.mode=0777; " + String(file.name()));
                    // Serial.print("  DIR : ");
                    // Serial.println(file.name());
                    // if(levels){
                    // 	listDir(fs, file.name(), levels -1);
                    ...
                    fn = file.name();
                    fs = String(file.size());
                    //Type = file; Size = 281816; Modify = 20200202063848; Perm = adfrw; UNIX.mode = 0666; hbmenu.nsp\r\n
                    data.println("Type=file;Size=" + fs + ";" + "modify=20000101160656;Perm=adfrw;UNIX.mode=0666; " + fn);
                    nm++;
                }
                file = root.openNextFile();
            }
    ...
    {
        if (SD.mkdir(String(cwdName) + parameters)) {
            //257 "b" directory created.\r\n
            client.println("257 \"" + String(parameters) + "\" directory created.");  //not support on espyet
        }
        else {
    ...
    {
        if (SD.rmdir(String(cwdName) + parameters)) {
